...
- **LangGraph orchestration** (feature-flagged via `RAG_LANGGRAPH_ENABLED=false`): When enabled, `_try_generate_next()` delegates to `app.services.langgraph_rag_workflow.run_rag_graph` — a 14-node StateGraph: `gate_check → query_planner → retrieve → reranker → context_compressor → evidence_gate → retrieval_repair → generate → grounding_validator → validate → distractor_validator → duplicate_gate → confidence_gate → persist`. On insufficient evidence, the graph retries once via `retrieval_repair` (broader queries) before aborting. When disabled (default), the legacy inline path in `_try_generate_next()` runs the same steps sequentially without LangGraph.
